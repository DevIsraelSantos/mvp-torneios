...
export default function NewTournamentPage() {
  ...
  const handleAddCourt = () => {
    ...
    const courtExists = courts.some((court) => court.name.toLowerCase() === courtName.toLowerCase())
  ...
  return (
    <div className="container mx-auto py-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">Preencha os dados para criar um novo torneio</p>
      </div>
          ...
          <CardContent className="pt-6">
            <div className="space-y-6">
              <div className="grid gap-3">
                ...
                <Input id="name" placeholder="Ex: Torneio de Verão 2023" required />
              </div>

              <div className="grid gap-3">
                <Label htmlFor="sets">Quantidade de sets por jogo</Label>
                <Input id="sets" type="number" min="1" max="5" defaultValue="3" required />
              </div>

              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  <Label htmlFor="tiebreak">Tie-break</Label>
                  <p className="text-sm text-muted-foreground">Habilitar set de desempate</p>
                </div>
                <Switch id="tiebreak" checked={hasTieBreak} onCheckedChange={setHasTieBreak} />
              </div>

              {hasTieBreak && (
                <div className="grid gap-3 pl-6 border-l-2 border-muted">
                  <Label htmlFor="tiebreak-points">Pontos para vencer o tie-break</Label>
                  <Input id="tiebreak-points" type="number" min="1" defaultValue="15" required />
                </div>
              )}
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="grid gap-3">
                    ...
                    <Input id="victory-points" type="number" min="0" defaultValue="3" required />
                  </div>
                  <div className="grid gap-3">
                    <Label htmlFor="defeat-points">Derrota</Label>
                    <Input id="defeat-points" type="number" min="0" defaultValue="1" required />
                  </div>
                  <div className="grid gap-3">
                    <Label htmlFor="wo-points">WO</Label>
                    <Input id="wo-points" type="number" min="0" defaultValue="0" required />
                  </div>
                  <div className="grid gap-3">
                    <Label htmlFor="oo-points">OO</Label>
                    <Input id="oo-points" type="number" min="0" defaultValue="0" required />
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <p className="text-sm text-muted-foreground">Habilitar jogo final</p>
                </div>
                <Switch id="final" checked={hasFinal} onCheckedChange={setHasFinal} />
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <p className="text-sm text-muted-foreground">Habilitar uso de múltiplas quadras</p>
                </div>
                <Switch id="multiple-courts" checked={hasMultipleCourts} onCheckedChange={setHasMultipleCourts} />
              </div>
              ...
              {hasMultipleCourts && (
                <div className="grid gap-4 pl-6 border-l-2 border-muted">
                  <div className="grid gap-3">
                    ...
                    <Input id="courts-count" type="number" min="1" value={courts.length} readOnly />
                  </div>
                    ...
                    <div className="space-y-2">
                      {courts.map((court) => (
                        <div key={court.id} className="flex items-center gap-2">
                          <Input value={court.name} readOnly className="flex-1" />
                          <Button
                            type="button"
                            variant="destructive"
                            size="icon"
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        className="flex-1"
                      />
                      <Button type="button" onClick={handleAddCourt} size="icon">
                        <Plus className="h-4 w-4" />
                      </Button>
                    </div>
                    {courtError && <p className="text-sm text-destructive">{courtError}</p>}
                  </div>
                </div>
              )}
